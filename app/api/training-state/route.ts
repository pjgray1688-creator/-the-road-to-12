...
export async function GET() {
  ...
  if (error) return privateJson({ error: "Training state unavailable" }, { status: 500 });
  const profile = read(data?.training_profile); const state = read(profile._trainingState);
  return privateJson({ sessionStatusOverrides: state.sessionStatusOverrides ?? {}, salvageAdjustments: state.salvageAdjustments ?? [] });
}
export async function POST(request: Request) {
  const supabase = await serverSupabase(); const { data: { user } } = await supabase.auth.getUser();
  if (!user) return privateJson({ error: "Authentication required" }, { status: 401 });
  const body = await request.json().catch(() => ({}));
  if (!body || typeof body !== "object") return privateJson({ error: "Invalid training state" }, { status: 400 });
  const { data: current, error: readError } = await supabase.from("profiles").select("training_profile").eq("id", user.id).maybeSingle();
  if (readError) return privateJson({ error: "Training state could not be saved" }, { status: 500 });
  ...
  if (error) return privateJson({ error: "Training state could not be saved" }, { status: 500 });
