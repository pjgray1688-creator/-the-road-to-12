...
export async function POST(request: Request) {
  ...
  if (!candidate?.id || !candidate.name || !Array.isArray(candidate.sets)) return privateJson({ error: "Invalid recovery candidate" }, { status: 400 });
  try {
    ...
    return privateJson({ error: status === 409 ? "A genuine Monday session already exists; no changes were made." : "Recovery could not be completed.", code }, { status });
  }
}
