...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!setId) return NextResponse.json({ error: "Invalid set" }, { status: 400 });
  try { return NextResponse.json(await deleteWorkoutSet(client, user.id, id, setId)); } catch { return NextResponse.json({ error: "Unable to delete set" }, { status: 500 }); }
}
