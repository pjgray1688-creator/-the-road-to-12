...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  catch (error) { const code = (error as { code?: string }).code ?? "DISCARD_FAILED"; return NextResponse.json({ error: "Unable to discard workout", code }, { status: code === "WORKOUT_NOT_ACTIVE" ? 409 : 500 }); }
}
