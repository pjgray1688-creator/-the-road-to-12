...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ organisationId: MADHOUSE_PRODUCTION_ORGANISATION_ID, results: await reconcileMadhouseCatalogue(repository) });
  } catch (error) {
    const operation = error instanceof Error && "operation" in error && typeof error.operation === "string" ? error.operation : "catalogue_bootstrap";
    console.error("[club-bootstrap] catalogue operation failed safely", { operation });
    return NextResponse.json({ error: "Catalogue bootstrap failed safely" }, { status: 500 });
  }
}
