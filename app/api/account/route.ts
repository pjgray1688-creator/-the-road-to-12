...
export async function GET() {
  ...
  return privateJson({ id: user.id, email: user.email, profile, siteUrl: siteUrl() });
}
...
export async function POST(request: Request) {
  ...
  if (error) { persistenceError("profile_update", error); return privateJson({ error: "Profile could not be updated", code: diagnosticCode(error, "update") }, { status: 500 }); }
  if (profile) return privateJson({ profile });
  ...
  if (insertError) { persistenceError("profile_upsert", insertError); return privateJson({ error: "Profile could not be updated", code: diagnosticCode(insertError, "upsert") }, { status: 500 }); }
  return privateJson({ profile: inserted });
}
