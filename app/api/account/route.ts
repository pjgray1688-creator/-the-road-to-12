...
export async function POST(request: Request) {
  ...
  if (error) { persistenceError("profile_update", error); return NextResponse.json({ error: "Profile could not be updated", code: "profile_update_failed" }, { status: 500 }); }
  ...
  if (insertError) { persistenceError("profile_upsert", insertError); return NextResponse.json({ error: "Profile could not be updated", code: "profile_update_failed" }, { status: 500 }); }
