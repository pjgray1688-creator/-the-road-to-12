...
export default function AccountPage() {
  ...
  if (account) return <main className="legal-page account-page"><header><span className="brand-mark">T/12</span><div><p className="eyebrow">THE ROAD TO 12%</p><h1>Account</h1></div></header><section className="card account-card"><span className="eyebrow">SIGNED IN</span><h2>Signed in as {account.email}</h2><p>{account.profile?.display_name || "Profile"}</p><p>Timezone: {account.profile?.timezone || "Europe/London"}</p><p>Step goal: {(account.profile?.step_goal ?? 10000).toLocaleString()}</p><p>WHOOP: managed from the Readiness card.</p><p>Data migration: available for your local-first records.</p><button className="secondary" onClick={signOut}>Sign Out</button></section><p><Link href="/">Back to app</Link></p>{message && <p>{message}</p>}</main>;
  return <main className="legal-page account-page"><header><span className="brand-mark">T/12</span><div><p className="eyebrow">THE ROAD TO 12%</p><h1>Account</h1></div></header><p>Create an account or sign in to keep your training data private and synced.</p><label>Email<input type="email" value={email} onChange={event => setEmail(event.target.value)} /></label><label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} /></label><div className="account-actions"><button className="primary" onClick={() => submit("signIn")}>Sign in</button><button className="secondary" onClick={() => submit("signUp")}>Create account</button></div><button className="text-button" onClick={reset}>Forgot password?</button>{message && <p>{message}</p>}</main>;
}
