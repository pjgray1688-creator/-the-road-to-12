...
export default function ResetPasswordPage() {
  ...
  return <main className="legal-page account-page"><header><span className="brand-mark">T/12</span><div><p className="eyebrow">THE ROAD TO 12%</p><h1>Set a new password</h1></div></header>{!ready ? <p>This reset link is invalid or has expired. Request a new one to continue.</p> : <><label>New password<input type="password" autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} /></label><label>Confirm password<input type="password" autoComplete="new-password" value={confirm} onChange={event => setConfirm(event.target.value)} /></label><button className="primary" disabled={saving} onClick={() => void submit()}>{saving ? "Updating…" : "Update password"}</button></>}{message && <p role="status">{message}</p>}<p><Link href="/account">Back to sign in</Link></p></main>;
}
