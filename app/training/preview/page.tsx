...
export default function TrainingPreviewPage() {
  const params = useSearchParams(); const occurrenceId = params.get("occurrence"); const data = loadData();
  ...
  return <main className="app-shell module-page workout-preview"><PageHeader eyebrow="WORKOUT PREVIEW" title={occurrence.session.name} description={`${day} · ${occurrence.scheduledDate}`} /><Surface><span className="eyebrow">SCHEDULED SESSION</span><p className="muted">Preview only · {occurrence.status.replaceAll("_", " ")}</p>{exercises.map((exercise, index) => <div className="plan-row" key={exercise.id}><b>{String(index + 1).padStart(2, "0")}</b><span>{exercise.name}<small>{exercise.target} · {exercise.sets} working sets</small></span></div>)}</Surface><BackButton href="/training">Back to Training</BackButton><AppNav /></main>;
}
