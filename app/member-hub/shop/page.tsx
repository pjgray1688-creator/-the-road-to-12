...
export default async function MemberHubShop({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  const [localProducts, locations, balance, orders, promotionRows, supplierRows] = await Promise.all([repository.listCommerceProducts(organisationId), repository.listLocations(organisationId), repository.getMemberOperationalProfile(organisationId, user.id).then(profile => profile.customer ? repository.getBalanceAccountForCustomer(organisationId, profile.customer.id) : undefined), repository.listOrders(organisationId), client.from("club_promotions").select("id,status,starts_at,ends_at,location_ids,effects,eligibility").eq("organisation_id", organisationId).eq("status", "active"), client.rpc("club_list_member_supplier_catalogue", { p_organisation_id: organisationId, p_location_id: null })]);
  const supplierParents = Array.isArray(supplierRows.data) ? supplierRows.data as DurableSupplierParentRow[] : [];
  const supplierProducts = durableSupplierRowsToProducts(supplierParents, organisationId);
