...
export default async function ClubMembersPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  try {
    ...
    return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · MEMBERS" title="Members" description="Search people, memberships and access." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} /><div className="card"><a className="secondary" href={`/club/members/import?org=${encodeURIComponent(context.organisation.id)}`}>Import ClubManager members</a> <a className="secondary" href={`/club/induction?org=${encodeURIComponent(context.organisation.id)}`}>Inductions</a></div><ClubMemberOnboarding organisationId={context.organisation.id} customers={customers} products={products} role={context.role} canAssign={canAssign} /><ClubMembersDirectory members={summaries} customers={customers} organisationId={context.organisation.id} /><BackButton href={`/club?org=${encodeURIComponent(context.organisation.id)}`}>Back to Club</BackButton><AppNav /></AppShell>;
