...
import { memberAccessState, type ClubMemberSummary } from "@/lib/club-operational";
...
export default async function ClubMembersPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  try {
    ...
    const customerByUser = new Map(customers.filter(customer => customer.userId).map(customer => [customer.userId!, customer]));
    const summaries: ClubMemberSummary[] = context.members.map(member => { const customer = customerByUser.get(member.userId); return { ...member, displayName: customer?.displayName ?? "Club member", ...(customer?.email ? { email: customer.email } : {}), accessState: "needs_attention" }; });
    const theme = resolveOrganisationTheme(context.organisation);
    return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · MEMBERS" title={theme.organisationName} description="Find members and review operational access context." /><ClubMembersDirectory members={summaries} organisationId={context.organisation.id} /><BackButton href={`/club?org=${encodeURIComponent(context.organisation.id)}`}>Back to Club</BackButton><AppNav /></AppShell>;
