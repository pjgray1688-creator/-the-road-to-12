...
export async function syncActiveSportsCatalogueAction(input: { organisationId: string; csv: string; fileName: string }) {
  ...
  const { data: linked, error: linkError } = await client.rpc("club_link_supplier_catalogue_to_commerce", { p_organisation_id: input.organisationId, p_supplier_name: "Active Sports" });
  if (linkError) return { ok: false as const, error: "Import failed", diagnostic: { operation: "club_link_supplier_catalogue_to_commerce", sqlState: linkError.code, message: linkError.message, ...(linkError.details ? { detail: linkError.details } : {}), ...(linkError.hint ? { hint: linkError.hint } : {}) } };
  const result = (data ?? {}) as Record<string, unknown>;
  return { ok: true as const, updated: Number(linked?.updated ?? result.updated ?? 0), created: Number(linked?.created ?? result.created ?? 0), discontinued: Number(result.discontinued ?? 0), skipped: Number(result.invalid ?? 0), durationMs: Date.now() - started };
}
