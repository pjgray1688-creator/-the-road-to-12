...
import { isLegacyDemoCommerceProduct, sortCommerceProductsForOperations } from "@/lib/club-commerce";
...
export default async function ClubProductsPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  const localProducts = sortCommerceProductsForOperations(products.filter(product => product.active && !isLegacyDemoCommerceProduct(product)));
  const debug = { serverLoaded: products.length, serverGsn: products.filter(product => product.brand?.trim().toLowerCase() === "gsn" || product.category?.trim().toLowerCase().startsWith("gsn:")).length };
  const query = `?org=${encodeURIComponent(context.organisation.id)}`;
  return <AppShell className="module-page club-page"><p className="r12-shop-debug r12-shop-debug-banner">R12 SHOP DEBUG LIVE · route /club/products · organisation {context.organisation.id} · server commerce {debug.serverLoaded} · server GSN {debug.serverGsn} · active/archive/demo filtered {localProducts.length} · product search/render path ClubProductsPricing → Product Search</p><PageHeader eyebrow="R12 CLUB · COMMERCE" title="Products & Pricing" description="The management control centre for retail products, pricing and commercial health." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} /><ClubProductsPricing products={localProducts} diagnostics={debug} />{canManageSupplier ? pricing.error ? <EmptyState title="Supplier pricing is unavailable">Ask an administrator to check catalogue setup before importing.</EmptyState> : <ClubSupplierPricing organisationId={context.organisation.id} offers={offers} /> : null}<ClubCatalogue organisationId={context.organisation.id} products={localProducts} /><BackButton href={`/club${query}`}>Back to Club</BackButton><AppNav /></AppShell>;
}
