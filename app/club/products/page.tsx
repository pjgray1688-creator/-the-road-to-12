...
export default async function ClubProductsPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  const debug = context.organisation.id === "fa44592a-1593-4ad3-a621-63a4a4bcbceb" ? { serverLoaded: products.length, serverGsn: products.filter(product => product.brand?.trim().toLowerCase() === "gsn" || product.category?.trim().toLowerCase().startsWith("gsn:")).length } : undefined;
