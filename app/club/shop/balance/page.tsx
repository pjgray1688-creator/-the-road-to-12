...
export default async function BalanceTopUpPage({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string }> }) {
  ...
  const params = await searchParams; const [customers, locations] = await Promise.all([context.repository.listCustomers(context.organisation.id), context.repository.listLocations(context.organisation.id)]); const locationId = locations.some(item => item.id === params?.location && item.active) ? params?.location : locations.find(item => item.active)?.id;
  return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · BALANCE" title="Madhouse Balance" description="Record a cash top-up for a linked member." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} locations={locations} locationId={locationId} /><ClubBalanceTopUp organisationId={context.organisation.id} locationId={locationId} customers={customers} /><BackButton href={`/club/shop?org=${encodeURIComponent(context.organisation.id)}&view=sell`}>Back to Shop</BackButton><AppNav /></AppShell>;
}
