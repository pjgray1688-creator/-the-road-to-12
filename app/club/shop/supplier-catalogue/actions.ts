...
export async function importGsnCatalogueAction(input: { organisationId: string; rows: Array<{ name: string; brand?: string; range?: string; category?: string; description?: string; retailPriceMinor?: number; estimatedTradePriceExVatMinor?: number; estimatedTradePriceExVat?: number; costSource?: string; sourceUrl?: string; parentImageReference?: string; variantImageReference?: string; nutrition?: Record<string, unknown>; ingredients?: string; allergens?: string }> }) {
  const value = await authorised(input.organisationId, false);
  ...
  for (const row of input.rows) {
    const mapped = gsnCommerceProductInsert({ brand: row.brand ?? "GSN", range: row.range ?? "", name: row.name, category: row.category, description: row.description, retailPriceMinor: row.retailPriceMinor, estimatedTradePriceExVatMinor: row.estimatedTradePriceExVatMinor, estimatedTradePriceExVat: row.estimatedTradePriceExVat, costSource: row.costSource, sourceUrl: row.sourceUrl, parentImageReference: row.parentImageReference, variantImageReference: row.variantImageReference, nutrition: row.nutrition, ingredients: row.ingredients, allergens: row.allergens });
    ...
    const importedCost = mapped.cost_price_minor === null ? undefined : Number(mapped.cost_price_minor); const changed = !existing || Number(existing.sell_price_minor) !== sellPriceMinor || (importedCost !== undefined && Number(existing.cost_price_minor) !== importedCost) || (row.sourceUrl && currentMedia.sourceUrl !== row.sourceUrl) || (row.parentImageReference && currentMedia.parentImageReference !== row.parentImageReference) || (row.variantImageReference && currentMedia.variantImageReference !== row.variantImageReference) || (row.description && existing.description !== row.description) || (row.ingredients && (currentMedia.enrichment as Record<string, unknown> | undefined)?.ingredients !== row.ingredients) || (row.allergens && (currentMedia.enrichment as Record<string, unknown> | undefined)?.allergens !== row.allergens);
    if (!changed) { unchanged++; continue; }
    const product = await value.client.rpc("club_save_commerce_product", { p_id: existing?.id ?? null, p_organisation_id: input.organisationId, p_sku: existing?.sku ?? null, p_barcode: existing?.barcode ?? null, p_name: String(existing?.name ?? mapped.name), p_brand: String(existing?.brand ?? mapped.brand), p_description: row.description ?? existing?.description ?? null, p_category: String(existing?.category ?? mapped.category), p_active: existing?.active !== false, p_stock_tracked: existing?.stock_tracked !== false, p_sell_price_minor: sellPriceMinor, p_cost_price_minor: importedCost ?? existing?.cost_price_minor ?? null, p_currency: String(existing?.currency ?? mapped.currency), p_tax_code: existing?.tax_code ?? null, p_supplier_reference: null, p_media: mergedMedia });
