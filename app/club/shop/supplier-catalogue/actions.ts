...
export async function importGsnCatalogueAction(input:{organisationId:string;rows:Array<{name:string;brand?:string;range?:string;category?:string;description?:string;retailPriceMinor?:number}>}){const value=await authorised(input.organisationId,false);if(!value||!input.rows.length)return{ok:false,error:"GSN import access required."};let created=0;for(const row of input.rows){const existing=(await value.client.from("club_commerce_products").select("id").eq("organisation_id",input.organisationId).eq("supplier_reference",`GSN|${row.range??""}|${row.name}`).maybeSingle()).data;if(existing)continue;const product=await value.client.from("club_commerce_products").insert({organisation_id:input.organisationId,name:row.name,brand:row.brand??"GSN",category:row.range?`GSN: ${row.range}`:(row.category??"GSN"),supplier_reference:`GSN|${row.range??""}|${row.name}`,supplier_member_orderable:false,supplier_availability_status:"unknown",description:row.description??null,active:true,stock_tracked:true,sell_price_minor:row.retailPriceMinor??0,currency:"GBP"}).select("id").single();if(product.error)return{ok:false,error:"GSN import failed."};created++;}revalidatePath("/club/shop");revalidatePath("/member-hub/shop");return{ok:true,created};}
