...
export async function commitCatalogueEnrichmentAction(input: { organisationId: string; rows: Array<Record<string, unknown>> }) {
  ...
  const payload = input.rows.filter(row => typeof row.variantId === "string" && row.variantId).map(row => ({ organisation_id: input.organisationId, supplier_product_id: row.variantId, description: row.description ?? null, nutrition: row.nutrition ?? null, ingredients: row.ingredients ?? null, allergens: row.allergens ?? null, parent_image_url: row.parentImageUrl ?? null, variant_image_url: row.variantImageUrl ?? null, source_url: row.sourceUrl ?? null, source_type: row.sourceType ?? null, verified_at: row.verifiedAt ?? null }));
