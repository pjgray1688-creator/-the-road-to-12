...
export default async function SupplierCataloguePage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · CATALOGUE" title="Supplier catalogue" description="Review grouped supplier variants, pricing and product information." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} /><Surface><ClubCatalogueOperations organisationId={context.organisation.id} products={products.map(product => ({ id: product.id, name: product.name, brand: product.brand, sku: product.sku, barcode: product.barcode, sellPriceMinor: product.sellPriceMinor, stockTracked: product.stockTracked }))} balances={[]} /><ClubCatalogueImport organisationId={context.organisation.id} products={products.map(product => ({ id: product.id, name: product.name }))} /><ClubSupplierCatalogue organisationId={context.organisation.id} offers={offers} products={products} /></Surface><AppNav /></AppShell>;
}
