...
async function loadShop(client: Awaited<ReturnType<typeof serverSupabase>>, userId: string, organisationId?: string, locationId?: string) {
  ...
  return { organisation, products, locations, activeLocation, balance, orders, declarations, customers, staff, canRecordCash, canReconcile, role: member.role, contexts: context.availableContexts };
}
...
export default async function ClubShopPage({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string }> }) {
  ...
  return <AppShell className="module-page club-shop-page"><PageHeader eyebrow="R12 CLUB · COMMERCE" title={theme.organisationName} description="Reception sales, member purchases and cash verification." /><ClubSectionNav organisation={loaded.organisation} role={loaded.role} contexts={loaded.contexts} locations={loaded.locations} locationId={loaded.activeLocation?.id} /><>{loaded.staff ? <ClubStaffCheckout organisationId={loaded.organisation.id} products={loaded.products} locations={loaded.locations} customers={loaded.customers} canRecordCash={loaded.canRecordCash} currentLocationId={loaded.activeLocation?.id} /> : null}<ClubShop organisationId={loaded.organisation.id} userId={user.id} products={loaded.products} locations={loaded.locations} balance={loaded.balance} orders={loaded.orders.filter(order => order.userId === user.id)} declarations={loaded.declarations} staff={loaded.staff} canReconcile={loaded.canReconcile} accent={theme.primaryAccent} currentLocationId={loaded.activeLocation?.id} /></><BackButton href={`/club?org=${encodeURIComponent(loaded.organisation.id)}`}>Back to Club</BackButton><AppNav /></AppShell>;
}
