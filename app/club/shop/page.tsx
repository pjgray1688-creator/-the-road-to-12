...
async function loadShop(client: Awaited<ReturnType<typeof serverSupabase>>, userId: string, organisationId?: string, locationId?: string) {
  ...
  const debug = organisation.id === "fa44592a-1593-4ad3-a621-63a4a4bcbceb" ? { serverLoaded: localProducts.length + supplierProducts.length, serverGsn: localProducts.filter(item => item.brand?.trim().toLowerCase() === "gsn").length + supplierProducts.filter(item => item.brand?.trim().toLowerCase() === "gsn").length } : undefined;
