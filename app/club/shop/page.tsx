...
import { isLegacyDemoCommerceProduct, mergeSupplierPresentation, sortCommerceProductsForOperations } from "@/lib/club-commerce";
...
async function loadShop(client: Awaited<ReturnType<typeof serverSupabase>>, userId: string, organisationId?: string, locationId?: string) {
  ...
  const debug = { serverLoaded: localProducts.length + supplierProducts.length, serverGsn: localProducts.filter(item => item.brand?.trim().toLowerCase() === "gsn").length + supplierProducts.filter(item => item.brand?.trim().toLowerCase() === "gsn").length };
  return { organisation, products, debug, families, locations: authorisedLocations, activeLocation, stockBalances, balance, orders, declarations, customers, deliveries, promotions, staff, canRecordCash, canReconcile, canAdjustStock, canRemoveStock, canManageCatalogue, role: member.role, contexts: context.availableContexts };
}
...
export default async function ClubShopPage({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string; view?: string }> }) {
  ...
  const debugBanner = <p className="r12-shop-debug r12-shop-debug-banner">R12 SHOP DEBUG LIVE · route /club/shop?view={view} · organisation {loaded.organisation.id} · server commerce {loaded.debug.serverLoaded} · server GSN {loaded.debug.serverGsn} · active/archive/demo filtered {loaded.products.length} · card path {loaded.staff && view === "sell" ? "ClubStaffCheckout → ClubProductFamilySelector" : "ClubShopBase → ClubMemberShop → ClubProductFamilySelector"}</p>;
  return <AppShell className="module-page club-shop-page"><p className="r12-shop-debug r12-shop-debug-banner">R12 SHOP DEBUG LIVE · route /club/shop?view={view} · organisation {loaded.organisation.id} · server commerce {loaded.debug.serverLoaded} · server GSN {loaded.debug.serverGsn} · active/archive/demo filtered {loaded.products.length} · card path {loaded.staff && view === "sell" ? "ClubStaffCheckout → ClubProductFamilySelector" : "ClubShopBase → ClubMemberShop → ClubProductFamilySelector"}</p><PageHeader eyebrow="R12 CLUB · COMMERCE" title="Shop" description={loaded.staff ? "Sell, stock, catalogue and cash." : "Browse products and manage your purchases."} /><ClubSectionNav organisation={loaded.organisation} role={loaded.role} contexts={loaded.contexts} locations={loaded.locations} locationId={loaded.activeLocation?.id} /><>{loaded.staff ? <ClubShopTabs organisationId={loaded.organisation.id} locationId={loaded.activeLocation?.id} view={view} /> : null}</><>{loaded.staff && view === "sell" ? <ClubStaffCheckout organisationId={loaded.organisation.id} products={loaded.products} families={loaded.families} locations={loaded.locations} customers={loaded.customers} canRecordCash={loaded.canRecordCash} currentLocationId={loaded.activeLocation?.id} diagnostics={loaded.debug} /> : null}{!loaded.staff && view === "sell" ? <ClubShop organisationId={loaded.organisation.id} userId={user.id} products={loaded.products} locations={loaded.locations} balance={loaded.balance} orders={loaded.orders.filter(order => order.userId === user.id)} declarations={[]} staff={false} canReconcile={false} accent={theme.primaryAccent} currentLocationId={loaded.activeLocation?.id} fulfilmentStates={fulfilmentStates} promotions={loaded.promotions} /> : null}{loaded.staff && view === "stock" ? <><ClubStockPanel organisationId={loaded.organisation.id} products={loaded.products} locations={loaded.locations} balances={loaded.stockBalances} locationId={loaded.activeLocation?.id} canAdjust={loaded.canAdjustStock} />{loaded.canRemoveStock ? <ClubStockRemoval organisationId={loaded.organisation.id} products={loaded.products} locationId={loaded.activeLocation?.id} /> : null}<ClubDeliveryHistory receipts={loaded.deliveries} locations={loaded.locations} /></> : null}{loaded.staff && view === "catalogue" && loaded.canManageCatalogue ? <><ClubCatalogue organisationId={loaded.organisation.id} products={loaded.products} /><ClubCatalogueImport organisationId={loaded.organisation.id} products={loaded.products} /></> : null}{loaded.staff && view === "cash" ? <ClubShop organisationId={loaded.organisation.id} userId={user.id} products={loaded.products} locations={loaded.locations} balance={loaded.balance} orders={loaded.orders.filter(order => order.userId === user.id)} declarations={loaded.declarations} staff={loaded.staff} canReconcile={loaded.canReconcile} accent={theme.primaryAccent} currentLocationId={loaded.activeLocation?.id} promotions={loaded.promotions} /> : null}</><BackButton href={`/club?org=${encodeURIComponent(loaded.organisation.id)}`}>Back to Club</BackButton><AppNav /></AppShell>;
}
