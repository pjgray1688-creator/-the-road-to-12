...
export default async function ClubInductionPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · INDUCTION" title="Gym induction" description="Configure the onboarding route members follow before unrestricted access." /><Surface><p className="muted">Induction is an auditable onboarding step. It does not replace the gym’s safety responsibilities.</p><InductionPolicyForm organisationId={context.organisation.id} /></Surface><BackButton href={`/club?org=${encodeURIComponent(context.organisation.id)}`}>Back to Club</BackButton><AppNav /></AppShell>;
}
