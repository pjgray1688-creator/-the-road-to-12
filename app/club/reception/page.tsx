...
async function loadReception(client: Awaited<ReturnType<typeof serverSupabase>>, userId: string, organisationId?: string, locationId?: string) {
  ...
  const [members, customers, products, locations, cashDeclarations, canAssign, canManageStaff] = await Promise.all([context.repository.listMemberSummaries(context.organisation.id), context.repository.listCustomers(context.organisation.id), context.repository.listProducts(context.organisation.id, true), context.repository.listLocations(context.organisation.id), context.repository.listCashDeclarations(context.organisation.id, "declared"), context.repository.hasCapability(context.organisation.id, userId, "memberships.assign"), context.repository.hasCapability(context.organisation.id, userId, "staff.permissions_manage")]);
  return { context, members, customers, products, locations, cashDeclarations, canAssign, canManageStaff, locationId: locations.some(location => location.active && location.id === locationId) ? locationId : context.role === "gym_staff" ? locations.find(location => location.active)?.id : undefined };
}
...
export default async function ClubReceptionPage({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string }> }) {
  ...
  const { context, members, customers, products, locations, cashDeclarations, canAssign, canManageStaff, locationId } = loaded;
  return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · RECEPTION" title="Reception" description="Find people, record memberships and keep the front desk moving." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} locations={locations} locationId={locationId} /><ClubReception organisationId={context.organisation.id} role={context.role} members={members} customers={customers} products={products} locations={locations} locationId={locationId} cashDeclarations={cashDeclarations} canAssign={canAssign} canManageStaff={canManageStaff} /><AppNav /></AppShell>;
}
