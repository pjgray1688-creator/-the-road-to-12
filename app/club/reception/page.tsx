...
export default async function ClubReceptionPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  const [members, customers, products, locations, cashDeclarations, canAssign] = await Promise.all([context.repository.listMemberSummaries(context.organisation.id), context.repository.listCustomers(context.organisation.id), context.repository.listProducts(context.organisation.id, true), context.repository.listLocations(context.organisation.id), context.repository.listCashDeclarations(context.organisation.id, "declared"), context.repository.hasCapability(context.organisation.id, user.id, "memberships.assign")]);
