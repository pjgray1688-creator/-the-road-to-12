...
async function loadReception(client: Awaited<ReturnType<typeof serverSupabase>>, userId: string, organisationId?: string) {
  ...
  return { context, members, customers, products, locations, cashDeclarations, canAssign };
}
...
export default async function ClubReceptionPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  let loaded;
  try {
    loaded = await loadReception(supabase, user.id, (await searchParams)?.org);
  } catch (error) {
  ...
  const { context, members, customers, products, locations, cashDeclarations, canAssign } = loaded;
  return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · RECEPTION" title="Reception" description="Find people, record memberships and keep the front desk moving." /><ClubSectionNav organisation={context.organisation} role={context.role} contexts={context.availableContexts} /><ClubReception organisationId={context.organisation.id} role={context.role} members={members} customers={customers} products={products} locations={locations} cashDeclarations={cashDeclarations} canAssign={canAssign} /><AppNav /></AppShell>;
}
