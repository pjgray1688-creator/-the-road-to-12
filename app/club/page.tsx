...
export default async function ClubPage({ searchParams }: { searchParams?: Promise<{ org?: string }> }) {
  ...
  try {
    ...
    const [members, locations, products, services, declarations, sessions, orders, customers] = await Promise.all([context.repository.listMemberSummaries(context.organisation.id), context.repository.listLocations(context.organisation.id), context.repository.listCommerceProducts(context.organisation.id), context.repository.listServices(context.organisation.id), context.repository.listCashDeclarations(context.organisation.id), context.repository.listClassSessions(context.organisation.id), context.repository.listOrders(context.organisation.id), context.repository.listCustomers(context.organisation.id)]);
