...
export async function generateMetadata({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string }> }): Promise<Metadata> {
  try {
    const supabase = await serverSupabase(); const { data: { user } } = await supabase.auth.getUser();
    if (!user) return clubMetadata("Overview");
    const contexts = await listClubOrganisationContexts(supabase, user.id); const params = await searchParams; const orgId = params?.org; const organisation = contexts.find(item => item.organisation.id === orgId)?.organisation ?? (contexts.length === 1 ? contexts[0].organisation : undefined);
    return clubMetadata("Overview", organisation?.name);
  } catch { return clubMetadata("Overview"); }
}

export default async function ClubPage({ searchParams }: { searchParams?: Promise<{ org?: string; location?: string }> }) {
  ...
  try {
    ...
    const params = await searchParams; const orgId = params?.org;
    ...
    const [members, locations, products, declarations, sessions, orders, customers, canAssign, canManageStaff] = await Promise.all([context.repository.listMemberSummaries(context.organisation.id), context.repository.listLocations(context.organisation.id), context.repository.listProducts(context.organisation.id, true), context.repository.listCashDeclarations(context.organisation.id), context.repository.listClassSessions(context.organisation.id), context.repository.listOrders(context.organisation.id), context.repository.listCustomers(context.organisation.id), context.repository.hasCapability(context.organisation.id, user.id, "memberships.assign"), context.repository.hasCapability(context.organisation.id, user.id, "staff.permissions_manage")]);
    ...
    const attention = [awaiting ? { label: `${awaiting} cash ${awaiting === 1 ? "declaration" : "declarations"} need verification`, detail: "Review the location cash box", href: `/club/payments?org=${encodeURIComponent(context.organisation.id)}&view=cash` } : null, disputes ? { label: `${disputes} cash ${disputes === 1 ? "dispute" : "disputes"} need review`, detail: "Resolve the recorded discrepancy", href: `/club/payments?org=${encodeURIComponent(context.organisation.id)}&view=cash` } : null, billingAttention ? { label: `${billingAttention} membership ${billingAttention === 1 ? "payment" : "payments"} need attention`, detail: "Review failed payments and recovery", href: `/club/payments?org=${encodeURIComponent(context.organisation.id)}` } : null, shelfAttention ? { label: `${shelfAttention} collection shelf ${shelfAttention === 1 ? "check" : "checks"} due`, detail: "Check parcels physically still present", href: `/club/shop/collections?org=${encodeURIComponent(context.organisation.id)}` } : null].filter(Boolean) as Array<{ label: string; detail: string; href: string }>;
    const isMadhouse = theme.organisationName.toLowerCase() === "madhouse gym";
    return <AppShell className="module-page club-page"><PageHeader eyebrow="R12 CLUB · OPERATIONS" title="Overview" description={`${theme.organisationName} operations at a glance.`} /><ClubSectionNav organisation={context.organisation} role={role} contexts={navContexts} locations={locations} locationId={params?.location} />{isMadhouse ? <div className={styles.brandBanner}><Image src="/club-branding/madhouse-club-wide.png" alt="R12 Club × Madhouse Gym" width={1200} height={240} priority sizes="(max-width: 760px) 100vw, 1180px" /></div> : null}<div className={styles.overview}><Surface className={attention.length ? styles.attention : ""}><span className="eyebrow">NEEDS ATTENTION</span>{attention.length ? <div className={styles.attentionList}>{attention.map(item => <Link className={styles.attentionItem} href={item.href} key={item.label}><span><strong>{item.label}</strong><small>{item.detail}</small></span><b aria-hidden="true">›</b></Link>)}</div> : <p className="muted" style={{ marginTop: 12 }}>Nothing needs attention right now.</p>}</Surface><Surface><span className="eyebrow">MEMBER SEARCH</span><h2>Find a member</h2><p className="muted">Search by name or email to open the existing member-management workflow.</p><ClubMembersDirectory members={members} customers={customers} organisationId={context.organisation.id} /></Surface><Surface><span className="eyebrow">QUICK ACCESS</span><div className={styles.quickGrid}><Link href={`/club/shop?org=${encodeURIComponent(context.organisation.id)}&view=sell`}><strong>Shop sale</strong><small>Open the staff till</small></Link><Link href={`/club/classes?org=${encodeURIComponent(context.organisation.id)}`}><strong>Classes</strong><small>{upcoming.length ? `${upcoming.length} coming up` : "No upcoming classes"}</small></Link><Link href={`/club/membership-billing?org=${encodeURIComponent(context.organisation.id)}`}><strong>Membership payments</strong><small>Review billing and recovery</small></Link><Link href={`/club/induction?org=${encodeURIComponent(context.organisation.id)}`}><strong>Inductions</strong><small>Manage member inductions</small></Link><Link href={`/club/shop/collections?org=${encodeURIComponent(context.organisation.id)}`}><strong>Collections</strong><small>Check orders awaiting action</small></Link>{canManageStaff ? <Link href={`/club/staff?org=${encodeURIComponent(context.organisation.id)}`}><strong>Staff</strong><small>Manage staff access and Club roles</small></Link> : null}</div></Surface><Surface><span className="eyebrow">MEMBER MANAGEMENT</span><h2>Member management</h2><p className="muted">Add or update a member and assign a membership or pass.</p><ClubMemberOnboarding organisationId={context.organisation.id} customers={customers} products={products} role={role} canAssign={canAssign} /></Surface><Surface><span className="eyebrow">UP NEXT</span>{upcoming.length ? upcoming.map(session => <div className="club-detail-row" key={session.id}><span>{session.title ?? "Scheduled class"}</span><span className="muted">{new Date(session.startsAt).toLocaleString("en-GB", { weekday: "short", hour: "2-digit", minute: "2-digit" })}</span></div>) : <p className="muted" style={{ marginTop: 12 }}>No upcoming classes scheduled.</p>}</Surface></div></AppShell>;
