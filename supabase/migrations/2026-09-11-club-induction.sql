...
declare v_policy record; v_version record; v_booking record; v_completion record; v_anchor timestamptz; v_due timestamptz; v_until timestamptz; v_state text; v_route text; v_days integer;
begin
  ...
  select min(m.starts_at) into v_anchor from public.club_memberships m join public.club_membership_holders h on h.membership_id=m.id and h.user_id=p_user_id where m.organisation_id=p_organisation_id;
  v_anchor:=coalesce(v_anchor,p_at); v_due:=v_anchor+(v_policy.grace_days||' days')::interval; v_route:=case when v_policy.requirement='in_person' then 'in_person' else 'online' end;
...
returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_version record; v_policy record;
begin
  ...
  select v.*,p.requirement into v_version from public.club_induction_versions v join public.club_induction_policies p on p.id=v.policy_id and p.organisation_id=v.organisation_id where v.id=p_version_id and v.organisation_id=p_organisation_id and v.status='published' and p.requirement='online_or_in_person';
  if not found or nullif(trim(p_acknowledgement_version),'') is null then raise exception 'Online induction is invalid' using errcode='22023'; end if;
...
returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_policy record; v_version record; v_booking public.club_induction_bookings%rowtype;
begin
  ...
  select v.* into v_version from public.club_induction_versions v where v.id=p_version_id and v.policy_id=v_policy.id and v.status='published';
