...
revoke all on table public.club_staff_permission_overrides from public, anon, authenticated;
revoke all on table public.club_audit_events from public, anon, authenticated;
...
returns boolean language sql stable security definer set search_path=pg_catalog,public as $$
  select case
    ...
    when p_capability not in ('members.view','members.create','members.link_account','memberships.assign','memberships.end_immediately','payments.take','payments.record_cash','refunds.issue','refunds.approve','cash.reconcile','inventory.adjust','staff.permissions_manage','induction.manage_policy','classes.manage','services.manage') then false
    ...
    else exists(select 1 from public.club_members m where m.organisation_id=p_organisation_id and m.user_id=auth.uid() and m.active and ((m.role='owner' and p_capability in ('members.view','members.create','members.link_account','memberships.assign','memberships.end_immediately','payments.take','payments.record_cash','refunds.issue','refunds.approve','cash.reconcile','inventory.adjust','staff.permissions_manage','induction.manage_policy','classes.manage','services.manage')) or (m.role='gym_admin' and p_capability in ('members.view','members.create','members.link_account','memberships.assign','memberships.end_immediately','payments.take','payments.record_cash','refunds.issue','refunds.approve','cash.reconcile','inventory.adjust','induction.manage_policy','classes.manage','services.manage')) or (m.role='gym_staff' and p_capability in ('members.view','members.create','members.link_account','payments.take','payments.record_cash','cash.reconcile')) or (m.role='trainer' and p_capability in ('members.view')))) end;
$$;
...
begin
 ...
 if p_capability not in ('members.view','members.create','members.link_account','memberships.assign','memberships.end_immediately','payments.take','payments.record_cash','refunds.issue','refunds.approve','cash.reconcile','inventory.adjust','staff.permissions_manage','induction.manage_policy','classes.manage','services.manage') then raise exception 'Unknown capability' using errcode='22023'; end if;
...
returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$ declare r public.club_audit_events%rowtype; begin if auth.uid() is null or not public.club_has_active_role(p_organisation_id,array['gym_staff','gym_admin','owner']) then raise exception 'Audit access denied' using errcode='42501'; end if; if p_location_id is not null and not exists(select 1 from public.club_locations l where l.id=p_location_id and l.organisation_id=p_organisation_id and l.active) then raise exception 'Location does not belong to organisation' using errcode='22023'; end if; insert into public.club_audit_events(organisation_id,actor_user_id,actor_role,action,target_type,target_id,location_id,reason,metadata) select p_organisation_id,auth.uid(),m.role,p_action,p_target_type,p_target_id,p_location_id,p_reason,coalesce(p_metadata,'{}'::jsonb) - 'actor_user_id' - 'organisation_id' - 'actor_role' - 'action' - 'target_id' - 'location_id' from public.club_members m where m.organisation_id=p_organisation_id and m.user_id=auth.uid() and m.active returning * into r; return to_jsonb(r); end; $$;
