...
function sessionRow(workout: Workout, userId: string, version = 1) {
  return {
    ...
    metadata: { substitutions: workout.substitutions ?? {}, ...(workout.workingTargets ? { workingTargets: workout.workingTargets } : {}), ...(workout.substitutionDetails ? { substitutionDetails: workout.substitutionDetails } : {}), notes: workout.notes ?? [], ...(workout.provenance ? { provenance: workout.provenance } : {}), ...(workout.recoveryEvidence ? { recoveryEvidence: workout.recoveryEvidence } : {}) }, updated_at: new Date().toISOString(),
  };
}
