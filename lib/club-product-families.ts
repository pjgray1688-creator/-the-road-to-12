import { canonicalSupplierFamilyBaseName, canonicalSupplierFamilyDisplayName, type ClubCommerceProduct } from "./club-commerce";
...
export function groupProductFamilies(products: ClubCommerceProduct[], families: ClubProductFamily[], organisationId: string, availability: Record<string, string> = {}): FamilyCard[] {
  ...
  for (const [groupKey, variants] of grouped) { const orderable = variants.some(v => availability[v.id] !== "UNAVAILABLE" && v.sellPriceMinor > 0); if (Object.keys(availability).length && !orderable) continue; const family = allowed.get(variants[0].familyId!)!; const ordered = variants.slice().sort(stable); const prices = [...new Set(ordered.filter(v => availability[v.id] !== "UNAVAILABLE").map(v => v.sellPriceMinor).filter(v => v > 0))].sort((a, b) => a - b); const supplierFamily = isSupplierFamilyProduct(ordered[0]); const format = supplierFamily ? supplierFormatClass(ordered[0]) : ""; const canonicalNames = supplierFamily ? new Set(ordered.map(product => canonicalSupplierFamilyBaseName(product.name))) : new Set<string>(); const sharedSupplierName = supplierFamily && canonicalNames.size === 1 && new Set(ordered.map(product => product.name)).size > 1 ? canonicalSupplierFamilyDisplayName(ordered[0].name) : family.name; const displayFamily = sharedSupplierName ? { ...family, name: sharedSupplierName } : family; const label = supplierFamily && format === "tub" ? displayFamily.name : supplierFamily || !formatKey(ordered[0]) ? displayFamily.name : `${displayFamily.name} · ${ordered[0].variantOptions?.size ?? ""}`; cards.push({ family: { ...displayFamily, id: groupKey }, variants: ordered, label, priceLabel: prices.length === 1 ? money(prices[0]) : prices.length ? `From ${money(prices[0])}` : "" }); }
