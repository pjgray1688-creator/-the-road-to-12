...
export function groupProductFamilies(products: ClubCommerceProduct[], families: ClubProductFamily[], organisationId: string): FamilyCard[] {
  ...
  for (const product of products.filter(p => p.organisationId === organisationId && selectable(p))) {
    if (product.familyId) { if (!allowed.has(product.familyId)) allowed.set(product.familyId, { id: product.familyId, organisationId, name: product.name, ...(product.brand ? { brand: product.brand } : {}), ...(product.category ? { category: product.category } : {}), active: true, sortPosition: 0 }); const gsnStandalone = product.brand?.toLowerCase() === "gsn" && !Object.keys(product.variantOptions ?? {}).length; const key = gsnStandalone ? `${product.familyId}::${product.id}` : `${product.familyId}::${formatKey(product)}`; const list = grouped.get(key) ?? []; list.push(product); grouped.set(key, list); } else ungrouped.push(product);
  }
