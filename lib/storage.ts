...
export function loadData(): AppData { if (typeof window === "undefined") return defaultData(); try { const data = JSON.parse(localStorage.getItem(KEY) || "null") ?? defaultData(); return { ...defaultData(), ...data, version: 2, workouts: data.workouts ?? [], bodyMetrics: data.bodyMetrics ?? [], meals: data.meals ?? [], recoverySnapshots: data.recoverySnapshots ?? [] }; } catch { return defaultData(); } }
