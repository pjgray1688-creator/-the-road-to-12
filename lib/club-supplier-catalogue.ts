...
const normaliseSupplierStockStatus = (value: unknown): SupplierStockStatus => { const status = String(value ?? "").trim().toLowerCase().replace(/[\u2013\u2014]/g, "-").replace(/\s+/g, " "); if (/^(available|in stock|in_stock|yes|true|1|orderable)$/.test(status)) return "available"; if (/^(unavailable|out of stock|out_of_stock|out-of-stock|oos|not available|not orderable|no|false|0|discontinued|unavailable - dated clearance)$/.test(status)) return "unavailable"; return "unknown"; };
const stock = (value: unknown): SupplierStockStatus => normaliseSupplierStockStatus(value);
