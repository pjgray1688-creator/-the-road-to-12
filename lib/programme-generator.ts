...
export type GeneratedProgramme = { id: string; name: string; profile: TrainingProfile; block: TrainingBlock; week: PlannedSession[]; rationale: string; previousProgrammes?: GeneratedProgramme[]; isLegacy?: boolean; framework?: string; conditioning?: import("./domain").ConditioningPrescription[]; generationVersion?: string };
...
function avoided(exercise: Exercise, profile: TrainingProfile) { const text = profile.limitations.concat(profile.dislikedExercises ?? []).join(" ").toLowerCase(); const knowledge = exerciseKnowledge(exercise.id); const pattern = `${exercise.name} ${knowledge?.movementPattern ?? ""} ${(knowledge?.primaryMuscles ?? []).join(" ")}`.toLowerCase(); return (text.includes("knee") && (pattern.includes("quad") || pattern.includes("leg") || /squat|lunge|knee/.test(pattern))) || (text.includes("shoulder") && pattern.includes("shoulder")) || profile.dislikedExercises?.some(item => item.toLowerCase() === exercise.id || item.toLowerCase() === exercise.name.toLowerCase()); }
