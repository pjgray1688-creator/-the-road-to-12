...
export function rampLoad(exercise: Exercise, current: number, target: number) { const safeCurrent = Number.isFinite(current) ? Math.max(0, current) : 0; const safeTarget = Number.isFinite(target) ? Math.max(0, target) : safeCurrent; if (safeCurrent >= safeTarget) return safeTarget; const profile = progressionProfile(exercise); if (profile.rampStyle === "plate_jump") { const distance = safeTarget - safeCurrent; const jump = distance >= safeTarget * .5 ? 40 : distance >= safeTarget * .2 ? 20 : distance >= safeTarget * .1 ? 10 : 5; return Math.min(safeTarget, snap(safeCurrent + jump, exercise, "up")); } return Math.min(safeTarget, next(safeCurrent, exercise)); }
