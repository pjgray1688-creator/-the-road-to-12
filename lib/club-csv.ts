...
export function parseCsvRecords(input: string, strict = false): Array<Record<string, string>> {
  ...
  for (let i = 0; i < input.length; i += 1) {
    ...
    if (quoted) {
      ...
      if (strict && (field.trim() || closedQuote)) throw new Error("Unexpected quote in CSV field");
      quoted = true;
      ...
      if (strict && closedQuote && ch.trim()) throw new Error("Unexpected text after CSV quote");
      field += ch;
    }
  }
  if (field || row.length) { row.push(field); if (row.some(value => value.trim())) rows.push(row); }
  if (strict && quoted) throw new Error("Unclosed CSV quote");
  if (!rows.length) return []; const headers = rows.shift()!.map(value => value.replace(/^\uFEFF/, "").trim().toLowerCase());
  if (strict && (new Set(headers).size !== headers.length || headers.some(header => !header))) throw new Error("Duplicate or empty CSV headers");
  if (strict && rows.some(values => values.length !== headers.length)) throw new Error("CSV row has the wrong number of columns");
