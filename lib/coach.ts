...
export function evaluateSet(exercise: Exercise, logged: LoggedSet[], feedback = "", previous: LoggedSet[] = [], position = 0): CoachDecision {
  ...
  const plan = startingPrescription(exercise, previous, position); const nonWorking = logged.filter(s => s.kind !== "working"); const baseline = workingWeight(exercise, previous); const priorWork = workingHistory(previous); const signal = effortSignal(feedback); const lowConfidence = priorWork.length === 0; const discoveryTarget = lowConfidence && latest.kind !== "working" && latest.reps >= high && signal === "easy" ? rampLoad(exercise, latest.weight, latest.weight + 40) : baseline; const target = latest.kind === "working" ? recalibratedTarget(exercise, previous, latest) : Math.max(preparationTarget(baseline, latest, exercise), discoveryTarget);
  ...
  if (latest.reps >= high && (latest.rir ?? 2) >= 2) { const next = (exercise.equipment === "barbell" || exercise.loadProfile === "plate_loaded") ? workingLoadIncrease(exercise, latest.weight, latest.rir) : magnitudeLoad(exercise, latest.weight, latest.reps, high, latest.rir); return { ...base, title: "Increase load", detail: `Go to ${next}kg for working set ${nextNumber} of ${exercise.sets}.`, nextWeight: next, tone: "progress", nextKind: "working", completed: false }; }
