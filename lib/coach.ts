...
import { magnitudeLoad, rampLoad, workingLoadIncrease } from "./progression";
...
export function restFor(exercise: Exercise, kind: SetKind, hard = false) { const base = kind === "warmup" ? 45 : kind === "ramp" ? 60 : exercise.restSeconds; if (!hard) return base; return kind === "working" ? Math.min(base + 30, 180) : Math.min(base + 30, kind === "ramp" ? 120 : 90); }
...
function recalibratedTarget(exercise: Exercise, previous: LoggedSet[], latest: LoggedSet, signal?: ReturnType<typeof effortSignal>) { const baseline = workingWeight(exercise, previous); const effort = latest.rir ?? 0; const { high } = parseRange(exercise.target); if (latest.weight < baseline * .75) return baseline; if ((exercise.equipment === "barbell" || exercise.loadProfile === "plate_loaded") && effort >= 5 && (signal === "easy" || latest.reps >= high + 3)) return grossUnderloadLoad(exercise, latest, high); if (effort >= 3 && latest.reps >= 8) return magnitudeLoad(exercise, latest.weight, latest.reps, high, effort); if (effort <= 0) return nextPracticalLoad(latest.weight, exercise, "down"); return snapAvailableLoad(latest.weight, exercise); }
export function evaluateSet(exercise: Exercise, logged: LoggedSet[], feedback = "", previous: LoggedSet[] = [], position = 0): CoachDecision {
  ...
  if (latest.kind === "ramp") { const rampCount = nonWorking.filter(s => s.kind === "ramp").length; const rampBudget = exercise.purpose === "isolation" || exercise.purpose === "core" ? 1 : priorWork.length > 0 ? 2 : 3; const enoughRamp = exercise.purpose === "isolation" || exercise.purpose === "core" || rampCount >= rampBudget || (rampCount >= plan.ramps && (latest.weight >= target || priorWork.length > 0)); const rampReps = rampCount <= 1 ? "5–8" : latest.weight >= target * .9 ? "1–3" : "3–5"; if (enoughRamp) return { ...base, title: "Working weight established", detail: `Go to ${target}kg. Working set 1 of ${exercise.sets}: aim for ${low}–${high} controlled reps.`, nextWeight: target, tone: "hold", nextKind: "working", completed: false }; const nextRamp = rampLoad(exercise, latest.weight, target); return { ...base, title: "Continue ramp", detail: `Go to ${nextRamp}kg for ${rampReps} reps, then we start working sets.`, nextWeight: nextRamp, repTarget: rampReps, tone: "hold", nextKind: "ramp", completed: false }; }
  ...
  if (latest.reps >= high && latest.rir !== undefined && latest.rir >= 2) { const plateLoaded = exercise.equipment === "barbell" || exercise.loadProfile === "plate_loaded"; const recalibrate = plateLoaded && latest.rir >= 5 && signal === "easy"; const next = recalibrate ? grossUnderloadLoad(exercise, latest, high) : plateLoaded ? workingLoadIncrease(exercise, latest.weight, latest.rir) : magnitudeLoad(exercise, latest.weight, latest.reps, high, latest.rir); return { ...base, title: recalibrate ? "Recalibrate working load" : "Increase load", detail: `Go to ${next}kg for working set ${nextNumber} of ${exercise.sets}.`, nextWeight: next, tone: "progress", nextKind: "working", completed: false }; }
