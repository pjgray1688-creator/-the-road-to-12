...
export function evaluateSet(exercise: Exercise, logged: LoggedSet[], feedback = "", previous: LoggedSet[] = [], position = 0): CoachDecision {
  ...
  if (latest.kind === "ramp") { const rampCount = nonWorking.filter(s => s.kind === "ramp").length; const enoughRamp = exercise.purpose === "isolation" || exercise.purpose === "core" || (rampCount >= plan.ramps && (latest.weight >= target || workingHistory(previous).length > 0)); if (enoughRamp) return { ...base, title: "Working weight established", detail: `Go to ${target}kg. Working set 1 of ${exercise.sets}: aim for ${low}–${high} controlled reps.`, nextWeight: target, tone: "hold", nextKind: "working", completed: false }; const nextRamp = rampLoad(exercise, latest.weight, target); return { ...base, title: "One final ramp", detail: `Go to ${nextRamp}kg for 4–6 reps, then we start working sets.`, nextWeight: nextRamp, repTarget: "4–6", tone: "hold", nextKind: "ramp", completed: false }; }
