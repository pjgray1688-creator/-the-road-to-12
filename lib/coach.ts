...
/** Only valid completed working sets are performance evidence. Preparation sets are never history. */
export const workingHistory = (previous: LoggedSet[]) => previous.filter(set => set.kind === "working" && Number.isFinite(set.weight) && set.weight > 0 && Number.isFinite(set.reps) && set.reps > 0);
export type WorkingLoadSuggestion = { suggestedLoad: number | null; confidence: "unknown" | "low" | "established"; basis: "no_history" | "history" | "user_confirmed"; previousWorkingSet?: LoggedSet; reasonCode: "no_history" | "repeat_last_successful_load" | "user_starting_weight" | "equipment_increment_adjustment" };
/** Free/tracker mode keeps load selection with the user. This is a suggestion,
 * never a logged set or an implicit target. */
export function resolveWorkingLoadSuggestion(exercise: Exercise, previous: LoggedSet[], userConfirmedTarget?: number): WorkingLoadSuggestion {
  if (Number.isFinite(userConfirmedTarget) && (userConfirmedTarget ?? 0) > 0) return { suggestedLoad: snapAvailableLoad(userConfirmedTarget!, exercise), confidence: "established", basis: "user_confirmed", reasonCode: "user_starting_weight" };
  const history = workingHistory(previous);
  if (!history.length) return { suggestedLoad: null, confidence: "unknown", basis: "no_history", reasonCode: "no_history" };
  const suggestedLoad = workingWeight(exercise, history);
  return { suggestedLoad, confidence: history.length >= 3 ? "established" : "low", basis: "history", previousWorkingSet: history.at(-1), reasonCode: "repeat_last_successful_load" };
}
...
export function evaluateSet(exercise: Exercise, logged: LoggedSet[], feedback = "", previous: LoggedSet[] = [], position = 0, experience?: "beginner" | "intermediate" | "experienced", targetOverride?: number): CoachDecision {
  ...
  const plan = startingPrescription(exercise, previous, position, targetOverride); const nonWorking = logged.filter(s => s.kind !== "working"); const baseline = Number.isFinite(targetOverride) && (targetOverride ?? 0) > 0 ? snapAvailableLoad(targetOverride!, exercise) : workingWeight(exercise, previous); const priorWork = workingHistory(previous); const signal = effortSignal(feedback); const calibration = calibrationSignal(feedback); const lowConfidence = priorWork.length === 0 && !targetOverride; const rampCountSoFar = nonWorking.filter(s => s.kind === "ramp").length; const discoveryTarget = lowConfidence && latest.kind !== "working" && calibration ? calibrationLoad(exercise, latest.weight, calibration, rampCountSoFar, experience ?? "intermediate") : baseline; const preparation = preparationTarget(baseline, latest, exercise); // A ramp is preparation for one already-resolved target: easy preparation must never move that target.
  const target = latest.kind === "working" ? recalibratedTarget(exercise, previous, latest, signal) : Math.max(preparation, Math.min(baseline, discoveryTarget));
  ...
  if (latest.kind === "ramp") { const rampCount = nonWorking.filter(s => s.kind === "ramp").length; const rampBudget = rampBudgetFor(exercise, priorWork.length > 0); const safetyStop = calibration === "too_hard"; const establish = calibration === "hard" || calibration === "getting_close"; const enoughRamp = safetyStop || establish || latest.weight >= target || rampCount >= rampBudget || (rampCount >= plan.ramps && priorWork.length > 0); const rampReps = rampRepTarget(exercise, rampCount, latest.weight >= target * .9); if (enoughRamp) { const workingTarget = safetyStop ? Math.min(target, calibrationLoad(exercise, latest.weight, "too_hard", rampCount, experience ?? "intermediate")) : target; return { ...base, title: safetyStop ? "Back off safely" : "Working weight established", detail: `Go to ${workingTarget}kg. Working set 1 of ${exercise.sets}: aim for ${low}–${high} controlled reps.`, nextWeight: workingTarget, tone: safetyStop ? "reduce" : "hold", nextKind: "working", completed: false }; } const nextRamp = rampLoad(exercise, latest.weight, target); return { ...base, title: calibration ? "How did that feel?" : "Continue ramp", detail: `Go to ${nextRamp}kg for ${rampReps} reps, then we start working sets.`, nextWeight: nextRamp, repTarget: rampReps, tone: "hold", nextKind: "ramp", completed: false }; }
