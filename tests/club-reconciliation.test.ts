...
const orders = [
  ...
  { id: "cancelled", created_at: "2026-09-01T12:30:00Z", status: "cancelled", subtotal_minor: 800, discount_minor: 0, total_minor: 800, currency: "GBP" }
];
...
test("reconciliation keeps order revenue distinct from tender and identifies exceptions", () => {
  ...
  assert.equal(summary.grossMinor, 1500);
  assert.equal(summary.discountMinor, 600);
  assert.equal(summary.netMinor, 900);
  ...
  assert.equal(summary.grossMinor, 1500, "cancelled orders are not revenue");
