...
test("programme snapshot uses dated occurrence outcomes when supplied", () => { const occurrences = week.map((session, index) => ({ occurrenceId: `active:${session.id}:2026-09-0${index + 1}`, programmeSessionId: session.id, scheduledDate: `2026-09-0${index + 1}`, status: index === 0 ? "completed" as const : "planned" as const, active: false, session, evidence: [] })); const snapshot = programmeSnapshot("Build Strength", currentBlock, week, [], true, occurrences); assert.equal(snapshot.completedSessions, 1); assert.equal(snapshot.nextSession?.id, "day-2"); });
