...
test("Rotherham Active Sports stocktake reconciles exact quantities by ledger delta", () => {
  ...
  assert.match(sql, /v_row\.qty-v_row\.current_qty/);
  assert.match(sql, /lower\(btrim\(name\)\)='rotherham'/);
  assert.doesNotMatch(sql, /carlton/);
  assert.match(sql, /Scitec Nutrition/);
  assert.match(sql, /v_row record/);
  assert.match(sql, /from resolved resolved_row/);
  assert.match(sql, /stock_rows as/);
  assert.doesNotMatch(sql, /\bfor r in\b|\bfrom resolved r\b|sm\.product_id=r\.product_id/);
  assert.doesNotMatch(sql, /set name='100% Whey Protein Professional'/);
...
test("member and reception call the canonical durable member catalogue RPC", async () => {
  ...
  assert.match(migration, /m\.role in \('member','gym_staff','gym_admin','owner'\)/);
  assert.doesNotMatch(migration, /commerce\.collections_manage/);
...
test("canonical commerce media accepts parent images and ignores empty media during merge", async () => {
  ...
  const supplier = { ...base, media: { url: "https://img.test/parent.jpg" } };
  assert.equal(mergeSupplierPresentation(base, supplier).media?.url, "https://img.test/parent.jpg");
});
...
test("supplier-linked products are stockable on the stock page even at zero balance", async () => {
  ...
  assert.equal(isStockableCommerceProduct(supplier), true);
  const fs = await import("node:fs/promises");
  assert.match(await fs.readFile("components/club-stock-panel.tsx", "utf8"), /isStockableCommerceProduct\(p\)/);
});
