...
import { cmToIn, defaultBodyUnits, displayCircumference, displayWeight, formatDisplayDate, inToCm, kgToLb, lbToKg, localCalendarDate } from "../lib/locale";
...
test("local calendar dates stay local when UTC crosses midnight", () => { const originalTimezone = process.env.TZ; process.env.TZ = "Pacific/Kiritimati"; try { const localLateNight = new Date("2026-09-20T10:30:00Z"); assert.equal(localCalendarDate(localLateNight), "2026-09-21"); assert.notEqual(localCalendarDate(localLateNight), localLateNight.toISOString().slice(0, 10)); } finally { if (originalTimezone === undefined) delete process.env.TZ; else process.env.TZ = originalTimezone; } });
test("local calendar dates remain padded and correct during daytime", () => { const daytime = new Date(2026, 0, 2, 13, 45); assert.equal(localCalendarDate(daytime), "2026-01-02"); assert.match(localCalendarDate(daytime), /^\d{4}-\d{2}-\d{2}$/); });
...
test("Progress uses authenticated server-first hydration and saves", () => { const source = fs.readFileSync("app/progress/page.tsx", "utf8"); const api = fs.readFileSync("app/api/progress/route.ts", "utf8"); assert.match(source, /fetch\("\/api\/progress"/); assert.match(source, /persistMetrics/); assert.match(api, /auth\.getUser/); assert.match(api, /_progressBodyMetrics/); });
test("Progress uses the local calendar helper for weight and measurement dates", () => { const source = fs.readFileSync("app/progress/page.tsx", "utf8"); assert.match(source, /const today = localCalendarDate/); assert.match(source, /date: weightDate \|\| today\(\)/); assert.match(source, /date: today\(\)/); assert.doesNotMatch(source, /today\s*=.*toISOString\(\)/); });
