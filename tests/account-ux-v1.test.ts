...
test("account presents consumer sections without backend or recovery jargon", () => {
  assert.match(source, /PROFILE/); assert.match(source, /Training preferences/); assert.match(source, /CONNECTIONS/); assert.match(source, /SUPPORT/);
...
test("account keeps canonical training and programme entry points", () => {
  assert.match(source, /href="\/training"/); assert.match(source, /href="\/onboarding"/); assert.doesNotMatch(source, /label="Progress"/); assert.match(source, /auth\.signOut/);
});
