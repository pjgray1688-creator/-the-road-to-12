...
import { canAccess, canAccessGym, canConsume, clubAcceptanceFixtures, consumeAllowance, grantAppliesToLocation, isValidAt, periodKey, remainingAllowance, resolveEntitlements, type EntitlementGrant, type EntitlementUsage } from "../lib/club";
...
test("location access policies keep snapshots separate from future locations", () => { const selected = grant({ entitlementKey: "gym_access", scope: "locations", locationIds: ["rotherham"] }); const currentSnapshot = grant({ entitlementKey: "gym_access", scope: "organisation", locationIds: ["rotherham", "carlton"] }); const future = grant({ entitlementKey: "gym_access", scope: "future_locations" }); assert.equal(grantAppliesToLocation(selected, "madhouse", "carlton"), false); assert.equal(grantAppliesToLocation(currentSnapshot, "madhouse", "future-site"), false); assert.equal(grantAppliesToLocation(future, "madhouse", "future-site"), true); });
...
test("memory repository is idempotent and organisation scoped", async () => { const repository = new MemoryClubRepository(); const products = await repository.listProducts(madhouseFixture.id, true); assert.equal(products.length, 12); const { id: _id, ...productWithoutId } = products[0]; const created = await repository.createProduct({ ...productWithoutId, name: "Gym Day Pass" }); assert.equal(created.id, products[0].id); assert.equal((await repository.listProducts("other-org")).length, 0); });
