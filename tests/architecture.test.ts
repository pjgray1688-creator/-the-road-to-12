...
test("future training rows open a preview without replacing Today", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); const preview = fs.readFileSync("app/training/preview/page.tsx", "utf8"); assert.match(training, /\/training\/preview\?occurrence/); assert.doesNotMatch(training, /href=\{`\/\?occurrence=/); assert.match(preview, /WORKOUT PREVIEW/); assert.match(preview, /resolveWeekSchedule/); });
