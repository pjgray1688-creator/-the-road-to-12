...
import { completedWorkoutsOnDate, occurrenceDateForDay, occurrenceKey, resolveToday, selectCompletedWorkout, uniqueCompletedSessionCount, upcomingAfterToday } from "../lib/schedule";
...
test("main app mounts visible recovery and check-in experience", () => { const source = fs.readFileSync("components/training-app.tsx", "utf8"); const dashboard = fs.readFileSync("components/dashboard-foundation.tsx", "utf8"); assert.match(source, /DashboardFoundation/); assert.match(dashboard, /readiness-card/); assert.match(dashboard, /DAILY CHECK-IN/); assert.match(dashboard, /How do you feel\?/); });
...
test("scheduled occurrence identity isolates repeated weekly sessions", () => { const first = occurrenceKey("tue", "2026-09-01"); const next = occurrenceKey("tue", "2026-09-08"); assert.notEqual(first, next); assert.equal(occurrenceDateForDay(new Date("2026-09-02T12:00:00Z"), "Europe/London", 2), "2026-09-01"); const upcoming = upcomingAfterToday(currentWeek, "Europe/London", new Date("2026-09-02T12:00:00Z"), 7); assert.equal(upcoming.find(item => item.id === "tue")?.scheduledDate, "2026-09-08"); });
...
test("ordinary sign-in redirects to Home while Account remains available", () => { const source = fs.readFileSync("app/account/page.tsx", "utf8"); assert.match(source, /router\.push\(next \?\? "\/"\)/); assert.match(source, /export default function AccountPage/); });
...
test("future training rows open a preview without replacing Today", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); const preview = fs.readFileSync("app/training/preview/page.tsx", "utf8") + fs.readFileSync("app/training/preview/preview-client.tsx", "utf8"); assert.match(training, /\/training\/preview\?occurrence/); assert.doesNotMatch(training, /href=\{`\/\?occurrence=/); assert.match(preview, /WORKOUT PREVIEW/); assert.match(preview, /resolveWeekSchedule/); });
test("Training rows are weekday-led without duplicated nested content", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); assert.doesNotMatch(training, /scheduledDate\.slice\(5\)/); assert.match(training, /weekday: "long"/); assert.doesNotMatch(training, /<div className="plan-row" key=\{occurrence\.occurrenceId\}/); });
test("Training presents the complete week and avoids a duplicate next-workout card", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); assert.match(training, /week\.filter\(session => session\.status === "rest"/); assert.match(training, /detail=\{occurrence\.status === "rest" \? "Rest"/); assert.doesNotMatch(training, /<span className="eyebrow">NEXT WORKOUT<\/span>/); });
test("server-selected programme identity prevents stale generated data replacing Original", () => { const storage = fs.readFileSync("lib/storage.ts", "utf8"); const shell = fs.readFileSync("components/home-shell.tsx", "utf8"); assert.match(storage, /activeProgrammeId === "legacy-personal-programme"/); assert.match(storage, /data\.generatedProgramme\?\.id !== data\.activeProgrammeId/); assert.match(shell, /activeProgrammeId: result\.active_programme_id/); });
test("Training keeps availability as one entry point and keeps the programme summary concise", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); const availability = fs.readFileSync("components/availability-settings.tsx", "utf8"); assert.match(training, /ACTIVE PROGRAMME/); assert.doesNotMatch(training, /snapshot\.goal\.replaceAll/); assert.match(availability, /Manage availability/); assert.doesNotMatch(availability.split("{open &&")[0], /Update this week/); assert.match(availability, /Make a change for this week/); });
test("top-level headings and Account identity avoid repeated labels", () => { const training = fs.readFileSync("app/training/page.tsx", "utf8"); const progress = fs.readFileSync("app/progress/page.tsx", "utf8"); const account = fs.readFileSync("app/account/page.tsx", "utf8"); const dashboard = fs.readFileSync("components/dashboard-foundation.tsx", "utf8"); assert.doesNotMatch(training, /PageHeader eyebrow="TRAINING"/); assert.doesNotMatch(progress, /PageHeader eyebrow="PROGRESS"/); assert.match(account, /PageHeader eyebrow="ACCOUNT" title=\{fullName\} \/>/); assert.match(account, /label="Personal details" detail=\{account\.email/); assert.match(dashboard, /today-greeting/); });
test("machine-branded exercise aliases retain canonical movement identity", () => { const workout = fs.readFileSync("lib/workout.ts", "utf8"); const types = fs.readFileSync("lib/types.ts", "utf8"); const history = fs.readFileSync("lib/training-history.ts", "utf8"); assert.match(workout, /name: "Leg Extension"/); assert.match(workout, /manufacturer: "Hoist"/); assert.match(types, /machineProfile/); assert.match(history, /Seated Machine Row/); });
test("stored branded overrides are canonicalised at every workout render boundary", () => { const workout = fs.readFileSync("lib/workout.ts", "utf8"); assert.match(workout, /canonicalExerciseName/); assert.match(workout, /name: canonicalExerciseName\(resolved\.name\)/); });
test("service worker uses an explicit public static-asset cache policy", () => { const sw = fs.readFileSync("public/sw.js", "utf8"); assert.match(sw, /r12-shell-v4/); assert.match(sw, /STATIC_ASSET_PATHS/); assert.match(sw, /request\.mode !== "navigate"/); assert.match(sw, /request\.destination !== "document"/); assert.match(sw, /isDocumentResponse/); assert.match(sw, /cacheControl\.includes\("private"\)/); assert.match(sw, /cacheControl\.includes\("no-store"\)/); assert.doesNotMatch(sw, /addAll\(\["\/"\]\)/); assert.doesNotMatch(sw, /caches\.match\("\/"\)/); assert.match(sw, /skipWaiting/); assert.match(sw, /clients\.claim/); });
