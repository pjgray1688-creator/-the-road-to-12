...
export function MissedSessionAction() {
  ...
  const review = () => setSalvage(planMissedSessionSalvage(today.session, activeWeek().filter(item => item.day > today.day && item.status === "planned"), data.workouts, data.trainingProfile ?? defaultTrainingProfile) ?? { title: "No catch-up recommended", detail: "I wouldn’t try to recover this session this week. Your next planned workout is the better place to continue.", additions: [], requiresApproval: true });
  const saveMissed = (reason?: string) => {
    ...
    saveData(next); setData(next); setReasonOpen(false); setConfirmOpen(false); setSalvage(planMissedSessionSalvage(today.session, activeWeek().filter(item => item.day > today.day && item.status === "planned"), data.workouts, data.trainingProfile ?? defaultTrainingProfile) ?? { title: "No catch-up recommended", detail: "I wouldn’t try to recover this session this week. Your next planned workout is the better place to continue.", additions: [], requiresApproval: true });
  };
  const apply = () => {
    if (!salvage) return;
