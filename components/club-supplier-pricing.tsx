...
import { normalizeCatalogueSearch } from "@/lib/club-commerce";
...
export function ClubSupplierPricing({ organisationId, offers, showImporter = false }: { organisationId: string; offers: SupplierPricingOffer[]; showImporter?: boolean }) {
  const [query, setQuery] = useState(""); const [category, setCategory] = useState(""); const [availability, setAvailability] = useState(""); const [review, setReview] = useState(""); const [page, setPage] = useState(1);
  const rows = useMemo(() => offers.filter(offer => [offer.name, offer.brand, offer.supplier, offer.variant, offer.size, offer.supplier_sku, offer.barcode, offer.category].some(value => normalizeCatalogueSearch(value ?? "").includes(normalizeCatalogueSearch(query))) && (!category || offer.category === category) && (!availability || offer.availability_status === availability) && (!review || (review === "Manual price" ? offer.manual_price : review === "Cost changed" ? offer.cost_history.length > 1 : health(offer) === review))), [offers, query, category, availability, review]);
const totalPages = Math.max(1, Math.ceil(rows.length / 10)); const currentPage = Math.min(page, totalPages); const pageRows = rows.slice((currentPage - 1) * 10, currentPage * 10);
  return <section className={styles.supplierPanel} aria-label="Supplier products and pricing"><h2>Supplier products &amp; pricing</h2><div className={styles.controls}><label>Find a supplier variant<input placeholder="Product, brand, supplier, SKU or barcode" value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} /></label><label>Category<select value={category} onChange={event => { setCategory(event.target.value); setPage(1); }}><option value="">All categories</option>{[...new Set(offers.map(offer => offer.category).filter((value): value is string => Boolean(value)))].sort().map(value => <option key={value}>{value}</option>)}</select></label><label>Supplier availability<select value={availability} onChange={event => { setAvailability(event.target.value); setPage(1); }}><option value="">All availability</option><option value="available">In stock</option><option value="unavailable">Unavailable</option><option value="unknown">Not checked</option></select></label><label>Pricing review<select value={review} onChange={event => { setReview(event.target.value); setPage(1); }}><option value="">All states</option>{["Healthy", "Price review", "Below margin floor", "Missing required data", "Supplier unavailable", "Manual price", "Cost changed"].map(value => <option key={value}>{value}</option>)}</select></label></div><p>{rows.length} variants found</p>{pageRows.map(offer => <Offer key={`${offer.id}-${offer.retail_price_minor}`} offer={offer} organisationId={organisationId} />)}{totalPages > 1 ? <div className="club-card-pagination" aria-label="Supplier pricing pagination"><button type="button" className="secondary" disabled={currentPage === 1} onClick={() => setPage(currentPage - 1)}>Previous</button><span>Page {currentPage} of {totalPages}</span><button type="button" className="secondary" disabled={currentPage === totalPages} onClick={() => setPage(currentPage + 1)}>Next</button></div> : null}{showImporter ? <ActiveSportsImport organisationId={organisationId} /> : null}</section>;
}
