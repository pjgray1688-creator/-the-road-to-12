...
export function ClubReception({ organisationId, role, members, customers, products, locations, cashDeclarations }: { organisationId: string; role: ClubRole; members: ClubMemberSummaryRead[]; customers: ClubCustomer[]; products: ClubProduct[]; locations: OrganisationLocation[]; cashDeclarations: CashItem[] }) {
  ...
  return <div className={styles.workspace}>
    ...
    <Surface><span className="eyebrow">COMMON TASKS</span><div className={styles.actions}><Link href={`/club/members?org=${encodeURIComponent(organisationId)}`}><strong>Add person</strong><small>Record a new walk-in or customer</small></Link><Link href={`/club/members?org=${encodeURIComponent(organisationId)}`}><strong>New membership</strong><small>Choose a real catalogue product</small></Link><Link href={`/club/shop?org=${encodeURIComponent(organisationId)}`}><strong>Shop sale</strong><small>Open the existing reception checkout</small></Link></div></Surface>
    ...
    <ClubMemberOnboarding organisationId={organisationId} customers={customers} products={products} role={role} />
  </div>;
}
