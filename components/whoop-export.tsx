...
export function formatWhoopExport(workout: Workout) { const lines = workout.sets.map(s => `${s.exerciseName} — ${s.weight}kg × ${s.reps}`); if (workout.cardio) lines.push(`Incline Treadmill — ${workout.cardio.duration} min — ${workout.cardio.incline}% incline — ${workout.cardio.speed.toFixed(1)} km/h`); return lines.join("\n"); }
