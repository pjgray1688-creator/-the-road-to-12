...
export function ClubMembersDirectory({ members, organisationId, customers = [] }: { members: ClubMemberSummaryRead[]; organisationId: string; customers?: Array<{ id: string; displayName: string; email?: string; userId?: string }> }) {
  ...
  const visibleCustomers = customers.filter(customer => !customer.userId && (!query || `${customer.displayName} ${customer.email ?? ""}`.toLowerCase().includes(query.trim().toLowerCase())) && accessFilter === "all");
  const total = visible.length + visibleCustomers.length;
  return <Surface><div className={styles.toolbar}><label><span className="sr-only">Search members</span><input aria-label="Search members" placeholder="Search by name or email" value={query} onChange={event => setQuery(event.target.value)} /></label><label><span className="sr-only">Filter access</span><select aria-label="Filter access" value={accessFilter} onChange={event => setAccessFilter(event.target.value)}><option value="all">All access</option><option value="active">Access active</option><option value="needs_attention">Needs attention</option><option value="unavailable">Access unavailable</option></select></label><label><span className="sr-only">Filter membership</span><select aria-label="Filter membership" value={membershipFilter} onChange={event => setMembershipFilter(event.target.value)}><option value="all">All memberships</option><option value="none">No membership</option>{statuses.map(status => <option value={status} key={status}>{membershipLabel(status)}</option>)}</select></label></div><p className={`muted ${styles.count}`}>{total} people</p>{total ? <div className={styles.list}>{visible.map(member => <a className={styles.row} key={member.id} href={`/club/members/${encodeURIComponent(member.userId)}?org=${encodeURIComponent(organisationId)}`}><span className={styles.avatar} aria-hidden="true">{member.displayName.trim().slice(0, 1).toUpperCase()}</span><span className={styles.main}><strong>{member.displayName}</strong><small>{member.email ?? "Member profile"}</small></span><span className={styles.meta}><small>{member.membershipName ?? "No current membership"}{member.homeLocation ? ` · Home: ${member.homeLocation.name}` : ""}</small><em data-state={member.accessState}>{accessLabel(member.accessState)}</em></span><b aria-hidden="true">›</b></a>)}{visibleCustomers.map(customer => <a className={styles.row} key={customer.id} href={`/club/members/customer/${encodeURIComponent(customer.id)}?org=${encodeURIComponent(organisationId)}`}><span className={styles.avatar} aria-hidden="true">{customer.displayName.trim().slice(0, 1).toUpperCase()}</span><span className={styles.main}><strong>{customer.displayName}</strong><small>{customer.email ?? "Customer record"}</small></span><span className={styles.meta}><small>No R12 account linked</small><em data-state="needs_attention">Account not linked</em></span><b aria-hidden="true">›</b></a>)}</div> : <p className="muted">{query || accessFilter !== "all" || membershipFilter !== "all" ? "No people match these filters." : "No people yet. Add a customer or member to begin."}</p>}</Surface>;
}
