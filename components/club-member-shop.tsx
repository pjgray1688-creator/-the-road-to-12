...
export function ClubMemberShop({add,cards,availability={}}:Props){const products=cards.flatMap(card=>card.variants);return <ClubProductFamilySelector products={products} families={cards.flatMap(card=>card.family?[card.family]:[])} organisationId={products[0]?.organisationId??""} availability={availability} cards={cards} onAdd={add}/>;}
