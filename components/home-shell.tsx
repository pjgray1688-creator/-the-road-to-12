...
export function HomeShell() {
  ...
  if (!completed && active) return <main className="shell home-screen"><DashboardFoundation workoutHistory={serverWorkouts} /><MissedSessionAction /><section className="card dashboard post-workout active-workout-card"><span className="eyebrow">WORKOUT IN PROGRESS</span><h2>{active.name}</h2><p>Your logged sets are saved. Resume when you&apos;re ready.</p><button className="primary big" onClick={() => setResumeRequested(true)}>Resume workout →</button></section><AppNav /></main>;
  if (!completed && !active && serverWorkouts.length === 0 && loadData().workouts.length === 0 && !loadData().generatedProgramme) return <main className="shell home-screen"><DashboardFoundation workoutHistory={serverWorkouts} /><MissedSessionAction /><section className="card dashboard"><span className="eyebrow">YOUR STARTING POINT</span><h2>Build your programme</h2><p>Answer a few quick questions and we&apos;ll shape your first training block.</p><a className="primary big" href="/onboarding">Build my programme <span>→</span></a></section><AppNav /></main>;
  if (!completed) return <><MissedSessionAction /><WorkoutErrorBoundary phase="preview" onHome={(current) => { if (current) setActive(current); setResumeRequested(false); }}><TrainingApp workoutHistory={serverWorkouts} onMinimize={(current) => { if (current) setActive(current); setResumeRequested(false); }} /></WorkoutErrorBoundary></>;
