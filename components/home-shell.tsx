...
export function HomeShell() {
  ...
  if (!resolved || !serverResolved) return <main className="shell" aria-busy="true"><PwaRegister /><section className="card dashboard"><span className="eyebrow">THE ROAD TO 12%</span><p>Loading today&apos;s plan…</p></section></main>;
  if (resumeRequested && active) return <TrainingApp resumeWorkout={active} workoutHistory={serverWorkouts} />;
  if (!completed && active) return <main className="shell"><PwaRegister /><DashboardFoundation workoutHistory={serverWorkouts} /><section className="card dashboard"><span className="eyebrow">WORKOUT IN PROGRESS</span><h2>{active.name}</h2><p>Your logged sets are saved. Resume when you&apos;re ready.</p><button className="primary big" onClick={() => setResumeRequested(true)}>Resume workout →</button></section></main>;
  if (!completed) return <TrainingApp workoutHistory={serverWorkouts} />;
  return <main className="shell"><PwaRegister /><DashboardFoundation workoutHistory={serverWorkouts} /><section className="card dashboard post-workout" aria-label="Post-workout"><span className="eyebrow">TODAY COMPLETE</span><h2>Nice work.</h2><p>{completed.name} is complete.</p><small>{summary?.sets ?? 0} working sets logged{summary?.cardio ? ` · ${summary.cardio} min cardio` : ""}.</small><p className="coach-note">Training is done for today. Easy mobility or a relaxed walk is plenty if you want to move.</p><div className="post-workout-actions"><button className="secondary" onClick={() => window.alert("Keep any extra activity easy and recovery-focused.")}>Ask Coach</button><button className="secondary" onClick={() => window.alert("Log a walk or mobility session from your activity tracker when available.")}>Recovery work</button></div></section></main>;
}
