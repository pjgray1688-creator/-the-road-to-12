...
import { localDateParts } from "@/lib/schedule";
import type { Workout } from "@/lib/types";

function completedToday(timezone: string): Workout | undefined {
  const today = localDateParts(new Date(), timezone).date;
  return loadData().workouts.find(workout => workout.completedAt && localDateParts(new Date(workout.completedAt), timezone).date === today);
}
