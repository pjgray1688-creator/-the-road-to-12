...
export function ClubSectionNav({ organisation, role, contexts = [], locations = [], locationId }: { organisation: Organisation; role: ClubRole; contexts?: ClubContextOption[]; locations?: Array<{ id: string; name: string; active: boolean }>; locationId?: string }) {
  ...
  return <nav className={styles.nav} aria-label="Club navigation"><div className={styles.identity}><div className={styles.tenant}><span className={styles.tenantMark}><BrandLockup compact /></span><span aria-hidden="true" className={styles.multiply}>×</span><div><span className="eyebrow">CLUB OPERATIONS</span><strong>{organisation.name}</strong></div></div><span className={styles.role}>{labelRole(role)}</span></div><div className={styles.links}>{links.map(([label, href]) => { const hrefPath = href.split("?")[0]; const active = hrefPath === "/club" ? pathname === "/club" : pathname.startsWith(hrefPath); return <Link href={href} key={href} aria-current={active ? "page" : undefined} className={active ? styles.active : undefined}>{label}</Link>; })}</div><div className={styles.contextControls}>{contexts.length > 1 ? <label className={styles.switcher}>Organisation<select aria-label="Choose organisation" defaultValue={organisation.id} onChange={event => router.push(`/club?org=${encodeURIComponent(event.target.value)}`)}>{contexts.map(context => <option key={context.organisation.id} value={context.organisation.id}>{context.organisation.name}</option>)}</select></label> : null}{activeVenues.length ? <label className={styles.switcher}>Venue<select aria-label="Choose operational venue" value={locationId ?? "all"} onChange={event => router.push(`${pathname}${query(event.target.value === "all" ? undefined : event.target.value)}`)}><option value="all">All locations</option>{activeVenues.map(location => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label> : null}</div></nav>;
}
